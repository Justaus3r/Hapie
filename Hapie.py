...
def HashWrite(string,file):
    ...
    #Bcrypt (can change every time due to random salt)
    hash_object_bcrypt_unfiltered = bcrypt.hashpw(string, bcrypt.gensalt())
    hash_object_bcrypt = hash_object_bcrypt_unfiltered.decode()
    #argon2
    Hash_obj = PasswordHasher()
    Hash_obj_argon2 = Hash_obj.hash(string)
    #sha256_crypt
...
if len(sys.argv) == 3 and sys.argv[1] == '-t':
    ...
    #MAybeNotCuzNowidId.....! yeah i know am lazy to define functions so i copied the code above for this argument too..
        string = (sys.argv[2])
        file = (sys.argv[4])
